...
import { courses, studentGrades, type IStudentGrade } from "./StudentGrade.type";
...
onMounted(() => {
    ...
    const course = courses.find((v) => v.id.toString() === courseId)
    ...
    <Dialog v-model:visible="modalProfile" modal :style="{ width: '40vw' }" :breakpoints="{ '764px': '90vw' }">
        <template #header>
            <div class="flex gap-2 align-items-center">
                ...
                <div class="h4 text-900 font-bold">{{ selectedProfile?.student.name }}</div>
            </div>
        </template>
        <div class="flex flex-column gap-2 py-2">
            <div class="flex flex-column gap-4 align-items-start">
                <template v-for="check in listCheckedData">
                    <div class="flex justify-content-between align-items-center w-full">
                        <template v-if="check.icon == 'note'">
                            <div class="flex gap-3">
                                ...
                                <h4 class="text-900 font-bold">{{ check.label }} <span class="text-green-300 font-bold">{{
                                    check.grade
                                }}%</span></h4>
                            </div>
                        </template>
                        <template v-if="check.icon === 'check'">
                            <div class="flex gap-3">
                                <!-- CHECK ICON -->
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                ...
                                <h4 class="text-900 font-bold">{{ check.label }}</h4>
                            </div>
                            <div class="flex gap-3" v-if="check.grade">
                                <h4 class="text-900 font-bold">Grade: <span class="text-green-300 font-bold">{{ check.grade
                                }}%</span>
                                </h4>
                            </div>
                        </template>
            ...
            <div class="flex align-items-center justify-content-between">
                <div class="flex justify-content-between align-items-center w-full">
                    <div class="flex gap-3">
                        ...
                        <h4 class="text-900 font-bold">Overall Grade</h4>
                    </div>

                    <div class="flex gap-3">
                        <h4 class="text-900 font-bold">Grade: <span class="text-green-300 font-bold">100%</span>
                        </h4>
                    </div>
                </div>
            </div>
