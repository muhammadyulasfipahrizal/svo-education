...
const removeItem = (groupIndex: number, itemIndex: number) => {
    materialData.value[groupIndex].items.splice(itemIndex, 1);
};
...
<template>
    <section class="my-3">
        ...
        <div v-for="(material, index) in materialData" :key="index">
            <div class="flex flex-column md:flex-row md:align-items-center mt-3 gap-2 px-3">
                <InputText class="text font-bold text-xl sm:text-2xl md:text-2xl md:h-3rem w-full"
                    v-model="material.heading" />
                <Button size="small" class="btn-add justify-content-center h-3rem md:hidden">
                    <i class="pi pi-plus"></i>
                    <p class="text-900 font-bold ml-2 text-white" @click="addMaterial(index)">ADD</p>
                </Button>
                <svg @click="addMaterial(index)" xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none" class="hidden md:block">
                ...
                <svg @click="removeMaterial(index)" xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none" class="hidden md:block">
                ...
                <ol>
                    <li class="text text-lg my-3" v-for="(item, itemIndex) in material.items" :key="itemIndex">
                        <div class="flex flex-column md:flex-row md:align-items-center mx-2 gap-2 md:gap-0">
                            ...
                            <InputText contenteditable class="w-full text text-lg" v-model="item.selectedItemURL" v-if="item.type === 'website'" style="height: 45px"/>
                            <InputText contenteditable class="w-full text text-lg" v-model="item.selectedItemURL" v-if="item.type === 'video'" style="height: 45px"/>
                            <InputText contenteditable class="w-full text text-lg" v-model="item.selectedItemURL" v-if="item.type === 'pdf'" style="height: 45px"/>
                            <svg @click="removeItem(index, itemIndex)" xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none" class="ml-2 hidden md:block">
                                <path d="M21.25 5V6.25H18.75V5H11.25V6.25H8.75V5C8.75 4.33696 9.01339 3.70107 9.48223 3.23223C9.95107 2.76339 10.587 2.5 11.25 2.5H18.75C19.413 2.5 20.0489 2.76339 20.5178 3.23223C20.9866 3.70107 21.25 4.33696 21.25 5Z" fill="#006785"/>
                                <path d="M25 7.5H5C4.66848 7.5 4.35054 7.6317 4.11612 7.86612C3.8817 8.10054 3.75 8.41848 3.75 8.75C3.75 9.08152 3.8817 9.39946 4.11612 9.63388C4.35054 9.8683 4.66848 10 5 10H6.25V25C6.25 25.663 6.51339 26.2989 6.98223 26.7678C7.45107 27.2366 8.08696 27.5 8.75 27.5H21.25C21.913 27.5 22.5489 27.2366 23.0178 26.7678C23.4866 26.2989 23.75 25.663 23.75 25V10H25C25.3315 10 25.6495 9.8683 25.8839 9.63388C26.1183 9.39946 26.25 9.08152 26.25 8.75C26.25 8.41848 26.1183 8.10054 25.8839 7.86612C25.6495 7.6317 25.3315 7.5 25 7.5ZM13.75 21.25C13.75 21.5815 13.6183 21.8995 13.3839 22.1339C13.1495 22.3683 12.8315 22.5 12.5 22.5C12.1685 22.5 11.8505 22.3683 11.6161 22.1339C11.3817 21.8995 11.25 21.5815 11.25 21.25V13.75C11.25 13.4185 11.3817 13.1005 11.6161 12.8661C11.8505 12.6317 12.1685 12.5 12.5 12.5C12.8315 12.5 13.1495 12.6317 13.3839 12.8661C13.6183 13.1005 13.75 13.4185 13.75 13.75V21.25ZM18.75 21.25C18.75 21.5815 18.6183 21.8995 18.3839 22.1339C18.1495 22.3683 17.8315 22.5 17.5 22.5C17.1685 22.5 16.8505 22.3683 16.6161 22.1339C16.3817 21.8995 16.25 21.5815 16.25 21.25V13.75C16.25 13.4185 16.3817 13.1005 16.6161 12.8661C16.8505 12.6317 17.1685 12.5 17.5 12.5C17.8315 12.5 18.1495 12.6317 18.3839 12.8661C18.6183 13.1005 18.75 13.4185 18.75 13.75V21.25Z" fill="#006785"/>
                            </svg>
                        </div>
