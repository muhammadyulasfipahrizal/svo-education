  ...
  <section class="max-h-full">
    ...
    <div class="badge-editor flex-col h-full md:flex-row mt-2" :style="`background: ${activeColor};`" style="border-radius: 10px;">
      <div class="bg-white flex justify-content-center align-items-center flex-column" style="gap: 15px;">
        <div class="relative">
          <Badge :color="activeColor" :width="isMobile ? '340' : '400'" :height="isMobile ? '340' : '400'" :word="firstLetterOfWord(title)" />
          <span class="title-badge"><em>{{ title }}</em></span>
        </div>
        <div class="flex justify-content-center badge-padding" style="gap: 8px;">
          <div v-for="color in colors" :key="color" role="button" :style="`background: ${color};`" @click="activeColor = color" :value="color" class="button-color"></div>
      ...
      <div style="padding: 0px 10px 10px 10px">
        <form class="flex flex-column" style="gap: 10px">
          <div class="flex flex-column gap-2" style="gap: 10px">
            ...
            <InputText type="text"  name="title" class="form-input" :value="title ? title : ''"
             placeholder="title" style="border-radius: 10px;"/>
          </div>
          <div class="flex flex-column" style="gap: 10px">
            <label for="title" class="form-label">Description</label>
            <Textarea type="text" rows="5" :value="description ? description : ''" name="title" auto-resize style="height: 100%; border-radius: 10px;" class="form-input" placeholder="title" />
          </div>
          <div class="flex justify-content-between" style="gap: 10px">
            ...
            <button class="button-form" @click="onSave">
...
const indexCostumize = ref<number | null>(null)
const selectedCostumize = ref<ICostumize>();

const color = ref(selectedCostumize.value?.color || '');
const title = ref(selectedCostumize.value?.title || '');
const description = ref(selectedCostumize.value?.description || '');

onMounted(() => {
    const selectedId = (route.params['id'] as string) as unknown as number;
    if (selectedId) {
        const findCostumizeIndex = costumizeStore.allCostumize.findIndex((v) => v.id == selectedId)
        if (findCostumizeIndex > -1) {
            const findCostumize = costumizeStore.allCostumize[findCostumizeIndex];
            indexCostumize.value = findCostumizeIndex;
            selectedCostumize.value = findCostumize;
            color.value = selectedCostumize.value.color;
            title.value = selectedCostumize.value.title;
            description.value = selectedCostumize.value.description;
            activeColor.value = selectedCostumize.value.color
        }
    }
})

const onSave = () => {
    if (indexCostumize.value !== null) {
        costumizeStore.updateCostumize(indexCostumize.value, { ...costumizeStore.allCostumize[indexCostumize.value], color: color.value, title: title.value, description: description.value })
